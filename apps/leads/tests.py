...
class TestImportFluentFormEntriesCommand(TestCase):
    """Test cases for import_fluentform_entries management command."""

    def test_import_fluentform_entries_json(self):
        import tempfile
        ...
        test_entries = [
            {
                ...
                "form_id": 3,
                "response": {
                    "names": {"first_name": "ياسر", "last_name": "علي"},
                    "email": "yaser_import@example.com",
                    "phone": "+201099998888",
                    "dropdown_1": "مصر",
                    "dropdown": "السعودية",
                    "dropdown_2": "ماجستير",
                    "address": "الرياض",
                    "hidden": "جامعة القاهرة",
                    "description": "طلب تسجبل جديد"
                }
            },
            {
                "id": 102,
                ...
                "form_id": 3,
                "response": {
                    "names": {"first_name": "قديم", "last_name": "جداً"},
                    "email": "old_import@example.com",
                    "phone": "+201000000000"
                }
            }
        ]
        ...
        with tempfile.NamedTemporaryFile('w+', delete=False, suffix='.json', encoding='utf-8') as tf:
            json.dump(test_entries, tf)
            tf_path = tf.name

        try:
            call_command('import_fluentform_entries', file=tf_path, since='2025-01-01')
            
            # The 2025 entry should be imported
            lead = Lead.objects.get(email='yaser_import@example.com')
            self.assertEqual(lead.name, 'ياسر علي')
            self.assertEqual(lead.phone, '+201099998888')
            self.assertEqual(lead.nationality, 'مصر')
            self.assertEqual(lead.residence_country, 'السعودية')
            self.assertEqual(lead.study_level, 'ماجستير')
            self.assertEqual(lead.address, 'الرياض')
            self.assertEqual(lead.institution_name, 'جامعة القاهرة')

            # The 2024 entry should NOT be imported
            self.assertFalse(Lead.objects.filter(email='old_import@example.com').exists())

        finally:
            if os.path.exists(tf_path):
                os.remove(tf_path)

    def test_import_fluentform_entries_sql(self):
        import tempfile
        from django.core.management import call_command

        sql_content = """
INSERT INTO `wp_fluentform_submissions` (`id`, `form_id`, `response`, `created_at`) VALUES
(201, 3, '{"names":{"first_name":"طارق","last_name":"حسن"},"email":"tarek_sql@example.com","phone":"+201111111111"}', '2025-03-10 14:00:00');
"""
        ...
        try:
            call_command('import_fluentform_entries', file=tf_path, since='2025-01-01')
            lead = Lead.objects.get(email='tarek_sql@example.com')
            self.assertEqual(lead.name, 'طارق حسن')
