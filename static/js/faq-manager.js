 ...
 * - عرض معاينة السؤال في الـ header
 * 
 * إعادة استخدام 70% من FacultyProgramsManager
 */
...
class FAQManager {
    ...
    attachAddHandler() {
        ...
        // ─── استيراد أسئلة جماعية ───
        const bulkImportBtn = document.getElementById('faq-bulk-import-btn');
        if (bulkImportBtn) {
    ...
    addFAQ() {
        ...
        // عمل collapse لكل الأسئلة الموجودة
        this.container.querySelectorAll('.faq-item:not(.faq-item--deleted)').forEach(item => {
            this.toggleAnswer(item, false);
        });
    ...
    createFAQItem(index) {
        ...
        item.innerHTML = `
            <div class="faq-item__header">
                <span class="faq-item__drag-handle" title="اسحب لإعادة الترتيب">
                ...
                <div class="faq-item__number">${index + 1}</div>
    ...
    reindexForms() {
        ...
        allItems.forEach((item, idx) => {
            item.setAttribute('data-faq-index', idx);
            const inputs = item.querySelectorAll('input, textarea');
            inputs.forEach(input => {
                const name = input.getAttribute('name');
                if (name) {
                    input.setAttribute('name', name.replace(/faqs-\d+-/, `faqs-${idx}-`));
                }
                const id = input.getAttribute('id');
                if (id) {
                    input.setAttribute('id', id.replace(/id_faqs-\d+-/, `id_faqs-${idx}-`));
                }
            });
            ...
            labels.forEach(label => {
                ...
                if (htmlFor) {
                    label.setAttribute('for', htmlFor.replace(/id_faqs-\d+-/, `id_faqs-${idx}-`));
                }
            });
        });
    ...
    // ─── تحديث الأرقام ───
    updateNumbers() {
        const visibleItems = this.container.querySelectorAll('.faq-item:not(.faq-item--deleted):not([style*="display: none"])');
        visibleItems.forEach((item, idx) => {
            const numberEl = item.querySelector('.faq-item__number');
    ...
    attachItemHandlers() {
        // أزرار الحذف
        this.container.querySelectorAll('.faq-item__delete').forEach(btn => {
            btn.onclick = (e) => {
                e.preventDefault();
        ...
        // أزرار التوسيع/الطي
        this.container.querySelectorAll('[data-toggle-answer]').forEach(btn => {
            btn.onclick = (e) => {
                e.preventDefault();
                const item = btn.closest('.faq-item');
        ...
        this.container.querySelectorAll('input[name$="-question"]').forEach(input => {
            input.addEventListener('input', (e) => {
                ...
                this.updateQuestionPreview(item);
            });
        });

        // السحب والإفلات للأسئلة
        this.container.querySelectorAll('.faq-item').forEach(item => {
            const handle = item.querySelector('.faq-item__drag-handle');
            if (handle) {
    ...
    onDragEnd(e, item) {
        item.classList.remove('faq-item--dragging');
        this.container.querySelectorAll('.faq-item--drag-over').forEach(el => {
            el.classList.remove('faq-item--drag-over');
        });
        this.updateSortOrders();
        this.reindexForms();
    }
    ...
    onDragOver(e, item) {
        ...
        if (!item.classList.contains('faq-item--dragging')) {
            item.classList.add('faq-item--drag-over');
        }
    }

    onDrop(e, item) {
        e.preventDefault();
        item.classList.remove('faq-item--drag-over');

        const draggedItem = this.container.querySelector('.faq-item--dragging');
        if (draggedItem && draggedItem !== item) {
            const allItems = [...this.container.querySelectorAll('.faq-item:not([style*="display: none"])')];
    ...
    // ═══════════════════════════════════════════════════════════════════════════════
    // استيراد أسئلة جماعية من كود Elementor Accordion
    // ═══════════════════════════════════════════════════════════════════════════════
    ...
    createBulkImportModal() {
        ...
        modal.id = 'faq-bulk-import-modal';
        modal.innerHTML = `
            ...
            <div class="faq-bulk-import-modal__content">
                <div class="faq-bulk-import-modal__header">
                    <h3>استيراد أسئلة من Elementor</h3>
                            ...
                            <div class="faq-bulk-import-preview">
                                <div class="faq-bulk-import-preview__stats">
                                    <div class="faq-bulk-import-stat">
                                        <span class="faq-bulk-import-stat__label">عدد الأسئلة:</span>
                                        <span class="faq-bulk-import-stat__value" data-faqs="0">0</span>
                                    </div>
                                </div>

                                <div class="faq-bulk-import-preview__list">
                                    <div class="faq-bulk-import-preview__title">الأسئلة المكتشفة:</div>
    ...
    parseElementorAccordion(htmlString) {
        try {
            ...
            if (!accordion) {
                return { faqs: [], errors: ['لم يتم العثور على Elementor Accordion في الكود'] };
            }
            ...
            accordion.querySelectorAll('.elementor-accordion-item').forEach((item, itemIndex) => {
                try {
                    ...
                    const question = titleEl ? titleEl.textContent.trim() : `سؤال بدون نص ${itemIndex + 1}`;

                    if (!question) {
                        errors.push(`السؤال ${itemIndex + 1}: لم يتم العثور على نص`);
                        return;
                    }

                    const contentEl = item.querySelector('.elementor-tab-content');
                    const answer = contentEl ? contentEl.textContent.trim() : '';

                    if (!answer) {
                        errors.push(`السؤال "${question}": لم يتم العثور على إجابة`);
                        return;
                    }

                    faqs.push({ question, answer });
                } catch (e) {
                    errors.push(`خطأ في معالجة السؤال ${itemIndex + 1}: ${e.message}`);
                }
            });

            if (faqs.length === 0) {
                return { faqs: [], errors: ['لم يتم العثور على أي أسئلة في الكود'] };
            }

            return { faqs, errors };
        } catch (e) {
            return { faqs: [], errors: [`خطأ في تحليل الكود: ${e.message}`] };
        }
    }

    updateBulkImportPreview() {
        const textarea = this.bulkImportModal.querySelector('.faq-bulk-import-textarea');
        const { faqs, errors } = this.parseElementorAccordion(textarea.value);

        this.bulkImportData = { faqs, errors };

        this.bulkImportModal.querySelector('[data-faqs]').textContent = faqs.length;

        const itemsList = this.bulkImportModal.querySelector('.faq-bulk-import-preview__items');
        itemsList.innerHTML = faqs.map(faq => `
            <li class="faq-bulk-import-preview__item">
                <span class="faq-bulk-import-preview__item-question">${this.escapeHtml(faq.question)}</span>
            </li>
        `).join('');
        ...
        const nextBtn = this.bulkImportModal.querySelector('.faq-bulk-import-btn-next');
        nextBtn.disabled = faqs.length === 0 || errors.length > 0;
    }
    ...
    confirmBulkImport() {
        if (!this.bulkImportData || this.bulkImportData.faqs.length === 0) {
            alert('لا توجد بيانات صحيحة للاستيراد');
            return;
        }

        this.bulkImportData.faqs.forEach(faq => {
            this.importFAQ(faq);
        });

        this.closeBulkImportModal();
        this.showBulkImportSuccess(`تم استيراد ${this.bulkImportData.faqs.length} سؤال بنجاح`);
    }
    ...
    importFAQ(faq) {
        ...
        if (questionInput) {
            ...
            this.updateQuestionPreview(newFAQItem);
        }
        if (answerInput) {
    ...
    showBulkImportSuccess(message) {
        ...
        messageDiv.className = 'faq-bulk-import-success-message';
        ...
        setTimeout(() => {
            messageDiv.classList.add('faq-bulk-import-success-message--hide');
