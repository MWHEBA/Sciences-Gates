/**
 * Custom HTML Editor
 * V1 Features: Bold, Italic, H2-H4, UL, OL, Link, Image
...
class CustomHTMLEditor {
    ...
    createToolbar() {
        ...
        const buttons = [
            ...
            { id: 'image', label: 'صورة', icon: '🖼️', command: 'insertImage', title: 'إدراج صورة' },
    ...
    handleImageInsert() {
        /**
         * Handle image insertion with alt text
         */
        const imageUrl = prompt('أدخل رابط الصورة:', 'https://');
        if (imageUrl) {
            const altText = prompt('أدخل نص بديل للصورة (للوصول):', '');
            
            // Create img tag with alt text
    ...
    sanitizeHtml(html) {
         ...
         * Future enhancements: video embeds, tables, CTA blocks
         */
        const allowedTags = ['p', 'br', 'strong', 'em', 'h2', 'h3', 'h4', 'ul', 'ol', 'li', 'a', 'img'];
...
document.addEventListener('DOMContentLoaded', function() {
    // Find all elements with class 'html-editor-widget'
    const editors = document.querySelectorAll('.html-editor-widget');
    
    editors.forEach(editor => {
        // Find the content area within the widget
        const contentArea = editor.querySelector('.editor-content');
        if (contentArea && !contentArea.hasAttribute('data-editor-initialized')) {
            new CustomHTMLEditor(contentArea);
            contentArea.setAttribute('data-editor-initialized', 'true');
        }
    });
});
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = CustomHTMLEditor;
}
