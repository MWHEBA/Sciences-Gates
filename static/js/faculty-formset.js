/**
 * Faculty Formset Manager
 * إدارة ديناميكية لقسم الكليات مع السحب والإفلات
 * 
 * Features:
 * - إضافة كليات جديدة ديناميكياً
 * - حذف كليات مع تأكيد
 * - إعادة ترتيب بالسحب والإفلات
 * - تحديث تلقائي لأرقام الترتيب
 * - أنيميشن سلسة
 */

class FacultyFormsetManager {
    constructor() {
    ...
    deleteFaculty(item) {
        ...
        // لو الكلية موجودة في الداتابيز (عندها id)، نعلمها للحذف
        if (idInput && idInput.value) {
            ...
            // كلية جديدة لسه ما اتحفظت — نشيلها من الـ DOM
            item.classList.add('faculty-item--deleted');
            setTimeout(() => {
                item.remove();
                this.reindexForms();
    ...
    // ─── إعادة ترقيم الفورمات بعد الحذف ───
    reindexForms() {
        const items = this.container.querySelectorAll('.faculty-item:not([style*="display: none"])');
        let formIndex = 0;

        // نحتاج نحسب كل العناصر بما فيها المحذوفة المخفية
    ...
    onDragOver(e, item) {
        ...
        // إزالة الـ indicator من كل العناصر
        this.container.querySelectorAll('.faculty-item--drag-over').forEach(el => {
            el.classList.remove('faculty-item--drag-over');
        });
    ...
    onDrop(e, item) {
        e.preventDefault();
        if (item === this.draggedItem || !this.draggedItem) return;
